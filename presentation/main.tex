...
\begin{frame}{\subsecname}
\begin{itemize}
    \item Eine \TeX\hyp{}Datei (\monospaced{.tex}) beginnt in der Regel mit einer sog.
        \alert{Präambel}:
    ...
    \item Notwendig: \lstinline{\\documentclass\{...\}}-Befehl zu Beginn zum Festlegen der
        ...
        \begin{itemize}
            \item Empfohlen: KOMA-Script-Klassen (\monospaced{scrartcl},
...
\begin{itemize}
    ...
    \item Möchte man unbedingt ein Leerzeichen erzeugen, so muss man eine Tilde einfügen:
        \code{whitespace_non-breaking.tex}
...
\begin{itemize}
    ...
    \item \dots~oder fragt uns eben um Hilfe.
...
\begin{frame}{\subsecname}
\begin{itemize}
    \item Mit dem Paket \monospaced{biblatex} kann man schnell und einfach Literaturverweise
        einfügen.
        \lstinputlisting[style=custom]{biblatex_preamble.tex}
    \item Die Daten der referenzierten \alert{Literatur} werden in einer
        \monospaced{.bib}\hyp{}Datei angegeben:
    ...
    \item Mit dem Befehl \lstinline{\\cite[<suffix>]\{<id>\}} erstellt man einen \alert{Verweis} auf
        ...
        \code{biblatex.tex}
    \item Die Zitierweise kann man anpassen (s. Dokumentation von \monospaced{biblatex}).
\end{itemize}
\end{frame}
